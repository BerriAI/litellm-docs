...
Four runtime controls decide how the proxy behaves when a provider is slow, a prompt is too large, a model name is wrong, or a Prometheus scrape lands mid-burst. They all live in `config.yaml` or the environment, and the [Helm charts and Terraform modules](#set-them-from-helm-and-terraform) expose each one as a typed value so you never edit a rendered manifest to turn one on. Sizing (workers, replicas, CPU and memory) is covered under [Sizing and workers](#sizing-and-workers) and is not repeated here.

### Keep streams alive across the ingress idle timeout

Every hop between the client and the proxy has an idle timeout: 60 seconds on an AWS ALB by default, 30 on a GCP external HTTP(S) load balancer's backend service, and whatever your ingress controller sets (`proxy-read-timeout` on ingress-nginx defaults to 60). A streaming request to a reasoning model can sit for longer than that before the first token, and when it does the load balancer closes the connection while the proxy and the provider are still working. The client sees a truncated or empty stream, and the proxy logs a successful upstream call.

The fix is to put bytes on the wire while the upstream is silent. For OpenAI-shaped streams such as `/v1/chat/completions`, and for pass-through routes, set `sse_keepalive_ping_interval_seconds` (proxy v1.98.0 or newer). The proxy then emits an SSE comment frame (`: ping`) every interval, including during the window before the upstream has answered at all, and every SSE client ignores comment frames by construction. It is off by default because opening the response before the upstream answers commits the `200` status early, so a failure after the first ping arrives as an SSE error frame rather than an HTTP error; see [Silence before the upstream answers](./timeout.md#silence-before-the-upstream-answers-at-all) for the details.

```yaml
litellm_settings:
  sse_keepalive_ping_interval_seconds: 15 # 1-300, well below the shortest idle timeout on the path
```

Pick a value comfortably below the shortest idle timeout on the path. With a 60 second ALB, 15 to 30 seconds is a good range; a value close to the timeout leaves no margin for scheduling jitter. `keepalive_seconds` on a single deployment overrides the global value for that deployment, and `keepalive_seconds: 0` hard-disables it there.

Anthropic-native streams (`/v1/messages`) already ping. Since v1.97.0 the proxy sends a keepalive every 15 seconds on those streams during upstream silence, matching what Anthropic's own API does, and `anthropic_sse_ping_interval_seconds` changes that interval:
...
### Reject oversized prompts before dispatch

Without a pre-call check, a prompt that exceeds the model's context window travels all the way to the provider, fails there, counts against your rate limit, and comes back as a provider error after a round trip. `enable_pre_call_checks` makes the router count the prompt tokens first and refuse the request with a `400 ContextWindowExceededError` before any provider call, or fall over to a deployment with a larger window when you set `context_window_fallbacks`:
...
The check is only as accurate as the window it compares against. For models in the [model cost map](https://github.com/BerriAI/litellm/blob/main/model_prices_and_context_window.json) the proxy knows `max_input_tokens`; for self-hosted or custom model names you must declare it under the deployment's `model_info`, otherwise the check has nothing to compare against and lets the request through:

```yaml
model_list:
  - model_name: internal-llm
    litellm_params:
      model: openai/internal-llm
      api_base: https://llm.internal
    model_info:
      max_input_tokens: 32000
```

The token count is an estimate from the proxy's tokenizer, not the provider's exact count, so leave a small margin rather than sizing prompts to the last token. Fallbacks and the full set of pre-call checks (region, allowed model groups) are in [Reliability: pre-call checks](./reliability.md#context-window-fallbacks-pre-call-checks--fallbacks).

### Invalid model names are cheap to reject

A request for a model name the proxy does not serve, or that the key is not allowed to call, is refused with a `400` or `403` and never reaches a provider. Before v1.101.0 a burst of those rejections (on the order of a hundred per second) could drive a worker to full CPU and raise latency for every other request on it, because each rejection re-sorted the router's wildcard pattern table and serialized the failure payload more than once. From v1.101.0 the rejection path does neither, rejections still show up in spend logs and the `litellm_proxy_failed_requests_metric` series, and there is nothing to configure. Run v1.101.0 or newer wherever clients are free to send arbitrary model names, and treat a sustained rate of invalid-model rejections as a client bug to chase rather than load to absorb.

### Keep Prometheus scrapes off the inference workers

`/metrics` on the proxy port is served by the same workers that serve inference. Rendering it aggregates every worker's sample files, and with high-cardinality labels one scrape can cost hundreds of milliseconds of CPU on the worker that received it, right in the request path. Set `PROMETHEUS_METRICS_PORT` (or `--prometheus_metrics_port`, v1.101.0 or newer) and the proxy serves the aggregated metrics from a separate process on that port, with `GET /health` on the same port for probes; `/metrics` stays on the proxy port for scrapers you have not moved yet. The separate port has no virtual-key authentication, so it stays on the cluster network behind a Service or security group rule and never behind the public ingress. Details, scrape config, and the Docker form are in [Serve /metrics from a separate process](./prometheus.md#serve-metrics-from-a-separate-process).

### Set them from Helm and Terraform

Both charts and both Terraform modules take the controls above as typed inputs and merge them into the generated `config.yaml`. A null or unset input leaves the proxy default alone, so upgrading changes nothing until you set a value, and a key you write directly into `proxy_config` always wins over the typed one.
...
reliability:
  sseKeepalivePingIntervalSeconds: 15
  anthropicSsePingIntervalSeconds: 10
...
The chart sets `PROMETHEUS_METRICS_PORT` on the container, adds a named `metrics` port to the pod, renders a dedicated `<release>-litellm-metrics` Service of type `ClusterIP` for it, and points the ServiceMonitor at that port instead of the proxy port. The main Service is left untouched, so setting `service.type: LoadBalancer` never publishes the unauthenticated metrics port. Rendering fails if `metricsServer.port` equals `service.port`. Requires `image.tag` v1.101.0 or newer (v1.98.0 if you only set the keepalive values).

</TabItem>
<TabItem value="helm-componentized" label="Helm (componentized litellm chart)">
...
Here the metrics server runs as a `metrics` sidecar container (same image, `python -m litellm.proxy.prometheus_metrics_server`) that shares the workers' `PROMETHEUS_MULTIPROC_DIR` over an `emptyDir`, with TCP readiness and liveness probes on the metrics port and its own `gateway.metricsServer.resources`. The port is exposed on a dedicated `<release>-litellm-gateway-metrics` `ClusterIP` Service rather than on the gateway Service, so point your scrape config at that Service. Each worker prunes the multiprocess files of workers that no longer exist when it starts and marks its own on shutdown, and the container entrypoint wipes the shared directory before Uvicorn forks, so a crashed or restarted worker does not leave stale gauge samples in the aggregate.
...
The module adds a nonessential `metrics` container to the gateway task with an ECS health check on the metrics port, a task volume for the shared multiprocess directory, and a security group ingress rule on that port for `gateway_metrics_scrape_cidrs` only. The ALB keeps routing port 4000 alone, so the metrics port is never reachable from the internet. `gateway_metrics_port` must not be 4000, and the keepalive values are validated to 1-300 at plan time. Setting any of these with an empty `proxy_config` still generates and uploads a `config.yaml`.
...
The GCP module merges the same values into the generated `config.yaml` but does not add a metrics port: the module gives the Cloud Run service one externally routed container port, so `/metrics` stays on port 4000 behind the internal load balancer. To take scrapes off the workers on Cloud Run, add the [Managed Service for Prometheus sidecar](https://cloud.google.com/stackdriver/docs/managed-prometheus/cloudrun-sidecar) to the service and have it scrape `localhost:4000/metrics`; Note that Cloud Run enforces its request timeout (300 seconds by default) on the whole response, so a keepalive keeps the internal load balancer from closing an idle stream but does not extend a stream past that limit.
