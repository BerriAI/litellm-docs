...
### Background jobs on multi-worker deployments
...
Jobs whose effect is shared, such as resetting budgets, pruning spend logs, rotating keys, or cleaning up expired Admin UI session keys, acquire a Redis lock (`PodLockManager`) before doing anything, so only one process runs them per cycle. Where Redis is not configured there is nothing to lock through, so each of them runs unguarded on every process that registered it. Configure Redis (see below) and keep `--num_workers 1` on Kubernetes so shared jobs run once across the deployment. There is no setting to restrict which jobs a given pod registers; every process registers every job.

Lock ownership is observable through the `litellm_pod_lock_manager_size` Prometheus gauge, which reports the number of locks held by the pod.
