...
A window rule can carry `override_dates`, a list of ISO `YYYY-MM-DD` dates read on the `weekday_timezone` calendar (default UTC). On a listed date only the rules that list it apply: their `weekdays` are ignored, and the flat `hours_utc` and every other rule are skipped. On any other date a rule with `override_dates` does not apply at all. Use it for public holidays, which are off-peak all day, and for make-up workdays, which follow the weekday windows even when they fall on a weekend
