...
Both modules take the runtime reliability controls as typed variables (`sse_keepalive_ping_interval_seconds`, `anthropic_sse_ping_interval_seconds`, `enable_pre_call_checks`, plus `gateway_metrics_port` and `gateway_metrics_scrape_cidrs` on AWS) and merge them into the generated `config.yaml`; see [Reliability controls](./prod.md#set-them-from-helm-and-terraform).
