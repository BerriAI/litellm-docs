...
| keepalive_seconds | Optional[float] | Send an SSE `: ping` comment on a streaming response whenever the upstream model goes silent for longer than this many seconds, repeating every `keepalive_seconds` until real content resumes. Use this to stop load balancers or reverse proxies from closing SSE connections that look idle during long silent gaps (e.g. extended thinking before the first visible token). Can be set per deployment or overridden per request with `keepalive_seconds` in the request body; a request can narrow the deployment's value (including disabling it with an explicit `0`), but if the deployment itself sets `0`, that's an operator-level disable a request can't override. Effective value is clamped to the range 1-300 seconds. Defaults to None (off). |
