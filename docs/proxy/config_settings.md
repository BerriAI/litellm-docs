...
| LITELLM_JOB_ROLE | Unreleased. This setting is not implemented on LiteLLM `main` or in a released version. Its implementation is under review in [LiteLLM PR #36618](https://github.com/BerriAI/litellm/pull/36618); setting it on a current version has no effect. See the [planned dedicated-worker topology](./prod.md#run-background-jobs-on-a-dedicated-worker).
