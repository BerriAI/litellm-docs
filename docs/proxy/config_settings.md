...
| SPEND_LOG_CLEANUP_STARTUP_DELAY_SECONDS | Seconds after proxy startup before the first interval-scheduled retention cleanup run (spend logs, health checks, autorouter sessions). Later runs follow `maximum_spend_logs_retention_interval`. Not used when `maximum_spend_logs_cleanup_cron` is set. Default is 60
| SPEND_LOG_CLEANUP_STARTUP_JITTER_SECONDS | Upper bound on the random extra delay added to `SPEND_LOG_CLEANUP_STARTUP_DELAY_SECONDS` so replicas that boot together do not all run the first cleanup at once. Default is 300
