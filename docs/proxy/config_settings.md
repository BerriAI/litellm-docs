...
general_settings:
  ...
  enforce_fallback_budget: boolean  # if true, router_settings fallbacks only run when the calling key and user are still within budget
...
| enforce_fallback_budget | boolean | Default `false`. When `true`, a fallback configured in `router_settings` only runs if the calling key and user are still within budget; over-budget targets are skipped and the primary model's error is returned when none remain. Zero-cost fallback targets are always allowed, and the primary attempt is never blocked. [More information here](reliability#enforce-budget-on-fallbacks) |
