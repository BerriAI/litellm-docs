...
| maximum_spend_logs_cleanup_batch_size | integer | Rows deleted per `DELETE` statement during spend log cleanup. Default is 1000. Environment equivalent of `SPEND_LOG_CLEANUP_BATCH_SIZE`. See [spend log deletion](spend_logs_deletion) |
| maximum_spend_logs_cleanup_max_batches | integer | Maximum `DELETE` statements issued per table per spend log cleanup run. Default is 500, so a run deletes at most 500,000 rows per table at the default batch size. Environment equivalent of `SPEND_LOG_RUN_LOOPS`. See [spend log deletion](spend_logs_deletion) |
| maximum_spend_logs_cleanup_run_budget | string | Wall-clock budget for an entire spend log cleanup run, shared across every table it cleans, as a duration string such as `5m`. Default is `5m`. When it is spent the run stops and resumes from the same cutoff on the next tick. Environment equivalent of `SPEND_LOG_CLEANUP_RUN_BUDGET_SECONDS`. See [spend log deletion](spend_logs_deletion) |
| maximum_spend_logs_cleanup_batch_timeout | string | Postgres `statement_timeout` and `lock_timeout` applied to each spend log cleanup batch, as a duration string such as `30s`. Default is `30s`, so no single delete can pin a lock while user traffic queues behind it. Environment equivalent of `SPEND_LOG_CLEANUP_BATCH_TIMEOUT_SECONDS`. See [spend log deletion](spend_logs_deletion) |
...
| SPEND_LOG_CLEANUP_RUN_BUDGET_SECONDS | Wall-clock budget in seconds for a whole spend log cleanup run, shared across every table it cleans. The run stops when the budget is spent and resumes from the same cutoff on the next tick. Environment equivalent of `general_settings.maximum_spend_logs_cleanup_run_budget`. Default is 300
| SPEND_LOG_CLEANUP_BATCH_TIMEOUT_SECONDS | Postgres `statement_timeout` and `lock_timeout` in seconds applied to each spend log cleanup batch, so no single delete holds a lock while user traffic queues behind it. Environment equivalent of `general_settings.maximum_spend_logs_cleanup_batch_timeout`. Default is 30
