...
| treat_finish_reason_as_failure | object | Map of terminal finish/stop reason to exception class name. A 200 response carrying a mapped reason counts as a deployment failure (allowed_fails, cooldown) and raises into the fallback chain when a fallback can serve. Non-streaming only. Example: `{"model_context_window_exceeded": "RateLimitError"}`. [Further docs](https://github.com/BerriAI/litellm/issues/38535) |
