...
| MCP_TRUSTED_NATIVE_REDIRECT_URIS | Comma-separated list of complete native MCP client callback URIs (custom `scheme://host/path`) accepted as `redirect_uri` by the MCP OAuth `authorize` endpoint in addition to the built-in `cursor://anysphere.cursor-mcp/oauth/callback`. An entry matches only when scheme, host and path are identical to the value the client sends. See [MCP OAuth: Redirect URLs for static clients](../mcp_oauth#static-client-redirect-urls).
