...
| enforce_rpm_tpm_on_model_add | boolean | Default `false`. When `true`, `POST /model/new` rejects a model whose `litellm_params` do not set both `rpm` and `tpm` to a positive value, returning a `400` instead of storing an unbounded deployment. Only model creation is checked; `POST /model/update` is unaffected. Available from `v1.100.0`. [Model management](model_management#automation-api) |
| enforce_project_model_quota | boolean | Default `false`. When `true`, `POST /project/new` and `POST /project/update` require a positive `model_rpm_limit` and `model_tpm_limit` entry for every model on the project and return a `400` otherwise. Updates are checked against the resulting project, so a partial update that leaves models and quotas untouched still passes. Projects that list `all-proxy-models`, a wildcard pattern, or a model access group are rejected while this is on, since those expand to several models at request time and cannot carry a per-model quota. Available from `v1.100.0`. [Project management](project_management#model-specific-quotas) |
