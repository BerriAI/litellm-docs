...
| enable_mid_stream_fallback_continuation | boolean | If true, a streaming chat completion that breaks after plain assistant text has been delivered continues on a prefill-capable fallback deployment via assistant prefill, instead of surfacing the error. Only deployments whose model supports assistant prefill are eligible. Async only. Default: false. |
