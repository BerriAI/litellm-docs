...
| optional_pre_call_checks | List[str] | List of pre-call checks to add to the router. Supported: `router_budget_limiting`, `prompt_caching`, `responses_api_deployment_check`, `encrypted_content_affinity` <SinceVersion v="1.82.3" />, `deployment_affinity`, `session_affinity`, `forward_client_headers_by_model_group` |
