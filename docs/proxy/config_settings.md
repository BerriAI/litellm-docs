...
| AGENT365_AGENT_IDENTITY_CLIENT_ID | Client id of the Entra agent identity the `agent_365` guardrail acts as when `auth_mode` is `agent_identity`
| AGENT365_AGENT_USER_UPN | User principal name of the agent user the `agent_365` guardrail evaluates as when `auth_mode` is `agent_identity`
