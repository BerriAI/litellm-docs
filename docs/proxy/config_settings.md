...
| autorouter_savings_baseline_model | string | Provider-qualified model (e.g. `anthropic/claude-opus-4-8`) that auto-router savings are measured against, as the single model you would have run instead of the router. When unset, each router derives its own baseline as the priciest model in its hardest configured tier. [Further docs](./auto_routing#reported-savings) |
