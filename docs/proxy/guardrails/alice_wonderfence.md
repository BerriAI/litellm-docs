...
2. The API key is configured at startup. The App ID is supplied **per virtual key / per team** — see [Multi-Tenant Setup](#multi-tenant-setup-per-app-credentials--policies).
...
> `app_id` is **not** an env var — it must be supplied per API key or per team (or per request, when `allow_request_metadata_override: true`).
...
Pin `alice_wonderfence_app_id` to a virtual key (recommended) or a team, then call the proxy with that key. See [Multi-Tenant Setup](#multi-tenant-setup-per-app-credentials--policies) for the full setup.

```bash
curl -X POST http://localhost:4000/v1/chat/completions \
  -H "Authorization: Bearer your-virtual-key-with-app-id-metadata" \
...
| `api_key` | No\* | `ALICE_API_KEY` env var | Default WonderFence API key. Overridable per virtual key / per team (and per request when `allow_request_metadata_override: true`). |
...
| `allow_request_metadata_override` | No | `false` | When `true`, allow per-request `metadata.alice_wonderfence_api_key` / `metadata.alice_wonderfence_app_id` as a last-resort source (after API-key and team metadata). Defaults to `false` so caller-controlled fields cannot bypass admin-pinned credentials. Enable only for trusted-gateway deployments. |

> \* `api_key` is required at runtime but does **not** need to be in the config if it will always be supplied per virtual key / per team. **`app_id` has no default** — it must always be supplied per virtual key or per team (see [Multi-Tenant Setup](#multi-tenant-setup-per-app-credentials--policies)).
...
Admin-pinned sources (virtual key and team metadata) always win over request metadata so a caller cannot bypass their assigned WonderFence app.

**`api_key` resolution** (with default fallback):

1. **Virtual key metadata** — set via `/key/generate`
2. **Team metadata** — set via `/team/new`
3. **Request metadata** — `metadata.alice_wonderfence_api_key` (only when `allow_request_metadata_override: true`)
...
1. **Virtual key metadata** — set via `/key/generate`
2. **Team metadata** — set via `/team/new`
3. **Request metadata** — `metadata.alice_wonderfence_app_id` (only when `allow_request_metadata_override: true`)
...
<TabItem value="per-request" label="Per Request (opt-in)">

Pass credentials in request metadata. Requires `allow_request_metadata_override: true` on the guardrail config — disabled by default so caller-controlled fields cannot bypass admin-pinned credentials.
...
Even with the flag on, virtual-key and team metadata still win over request metadata — the request value is only a last-resort source.
...
client = OpenAI(
    api_key="your-virtual-key-with-app-id-metadata",
...
| `true` | **Fail open.** Requests proceed without guardrail evaluation. An `ERROR` log line is emitted. The guardrail name is **not** added to the `x-litellm-applied-guardrails` response header so consumers can detect that scanning did not run. |
...
| 200 | Service error + `fail_open: true` | WonderFence unreachable but request proceeds (logged as `ERROR`; guardrail name omitted from applied-guardrails header) |
...
| `WARNING` | `DETECT` actions, sibling-stash recovery on post_call |
| `ERROR` | WonderFence unreachable (whether fail-open or fail-closed) |
...
**Error (HTTP 500):** `No alice_wonderfence_api_key found in API-key metadata, team metadata, request metadata (when allow_request_metadata_override is enabled), or default config (ALICE_API_KEY).`

Set the env var or supply per virtual key / per team metadata:
...
**Error (HTTP 500):** `No alice_wonderfence_app_id found in API-key metadata, team metadata, or request metadata (when allow_request_metadata_override is enabled). app_id must be provided per request.`

`app_id` has **no default**. Add it to virtual key metadata or team metadata (or request metadata when `allow_request_metadata_override: true`) — see [Multi-Tenant Setup](#multi-tenant-setup-per-app-credentials--policies).
