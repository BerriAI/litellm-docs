...
```python
fallbacks=[{"gpt-5.6-luna": ["gpt-5.6-terra"]}]
```
...
router = Router(
  ...
  fallbacks=[{"gpt-5.6-luna": ["gpt-5.6-terra"]}] # 👈 KEY CHANGE
)
...
```python
content_policy_fallbacks=[{"claude-sonnet-5": ["my-fallback-model"]}]
```
...
router = Router(
  ...
  content_policy_fallbacks=[{"claude-sonnet-5": ["my-fallback-model"]}], # 👈 KEY CHANGE
  # fallbacks=[..], # [OPTIONAL]
...
```python
context_window_fallbacks=[{"claude-sonnet-5": ["my-fallback-model"]}]
```
...
router = Router(
  ...
  context_window_fallbacks=[{"claude-sonnet-5": ["my-fallback-model"]}], # 👈 KEY CHANGE
  # fallbacks=[..], # [OPTIONAL]
...
litellm_settings:
  ...
  fallbacks: [{"zephyr-beta": ["gpt-5.6-luna"]}] # fallback to gpt-5.6-luna if call fails num_retries 
...
**Note:** This will only fallback to the model with the specific model ID. If you want to fallback to another model group, you can set `fallbacks=[{"gpt-5.6-terra": ["anthropic-claude"]}]`

2. Set fallbacks in config

```yaml
litellm_settings:
  fallbacks: [{"gpt-5.6-terra": ["my-specific-model-id"]}]
```
...
For example, a request to `gpt-5.6-luna` that fails over to `claude-fable-5` produces a row with `model_group=claude-fable-5`, `attempted_fallbacks=1`, and `original_model_group=gpt-5.6-luna`, so fallback-served and directly-served requests stay distinguishable after the fact:
...
If a request exceeds the limit, LiteLLM raises `ContextWindowExceededError` with details like `Model=gpt-5.6-terra, Max Input Tokens=10, Got=306`.
