...
{
  "error": {
    "message": "litellm.RateLimitError: RateLimitError: OpenAIException - Rate limit reached for gpt-4o-mini in organization org-abc on requests per min (RPM): Limit 3, Used 3.",
...
try:
    response = litellm.completion(model="gpt-4o-mini", messages=[{"role": "user", "content": "hi"}])
...
curl -X POST 'http://localhost:4000/key/generate' \
  ...
  -d '{"key_alias": "restricted", "models": ["gpt-4o-mini"]}'
