...
The consequence is the one an expert reader usually assumes is missing: "fix this typo" and "refactor the auth subsystem" are classified on those two short strings rather than on the 40k-token system prefix and reminder blocks they share, so they do not land on the same tier
...
Per-request cache tokens are not persisted. `LiteLLM_SpendLogs` carries `prompt_tokens`, `completion_tokens`, and `total_tokens` with no cache-token and no savings columns, so the cache split lives in the response usage object and the logging payload while the persisted split is the daily rollup above. Its `cache_hit` and `cache_key` columns are unrelated to any of this: they describe LiteLLM's own response cache, whether the proxy served this request from its cache rather than calling the provider, not provider-side prompt caching. What a single request does persist about routing is its `routing_decision` in metadata, including `routed_model`, `cause`, `tier`, `conversation_continuing`, and the baseline the deciding router recorded
