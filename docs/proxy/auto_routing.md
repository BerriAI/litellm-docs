...
The complexity router uses rule-based scoring (or an LLM classifier, or keyword rules) to tier requests and route to the right model per tier. Zero external API calls in the heuristic path; sub-millisecond latency.

Full reference and every v2 knob (LLM classifier, keyword rules, semantic matching, multi-model pools, adaptive Thompson sampling, session affinity, decision logging) lives on [Auto Router v2](./autorouter_v2.md).
