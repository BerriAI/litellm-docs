...
An auto-routed request records what routing saved against a counterfactual: the one model the traffic would have run on without a router. Nothing configures that model. The router derives it from its own tier ladder, because the ladder already names it. Without a router a deployment has to pick one model able to carry the hardest request it will see, so the baseline is the **most expensive model in the hardest tier the router configures**

Hardest configured, not `REASONING` unconditionally. A router that only defines `SIMPLE` and `MEDIUM` is measured against the best it could actually have picked, not against a frontier model it never had. When the tier entry names a pool rather than a single model, every model in that pool is a candidate; when a router defines its own tier labels, which carry no severity order, every model across every defined tier is a candidate and the cost ranking below picks from the whole set

"Most expensive" is settled by cost, not by rate. A model dearer per output token can be cheaper per cached token, so comparing a chosen pair of rates orders cache-heavy traffic backwards. Each candidate is instead priced once against a fixed reference request (20k prompt tokens, of which 19k are cache reads and 1k is a cache write, plus 1k completion) through the same cost engine the savings themselves use, which leaves cache rates, tiered tables and every other billing dimension to that engine. A candidate that prices to nothing there cannot stand in for what the traffic would have cost and is dropped; if none of them price, the driver reports zero rather than guessing. The ranking runs once per router instance and is pinned for its lifetime, so it stays off the per-request path; editing the router rebuilds the instance and re-derives it

The saving itself is a subtraction, and only one side of it is hypothetical. What the request actually cost is read back from what the cost calculator already recorded for it, on the same service tier and data residency it was priced on, rather than re-derived; a request billed at a priority tier and recomputed at standard rates would read as half its real cost. That recorded figure is the request's input and output cost, so built-in tool cost, discounts and margins sit outside the comparison, on the principle that they land on both arms or neither. The baseline never ran, so it is priced through that same engine on that same basis

The value is `0.0` rather than a number whenever the comparison would be meaningless: when either side cannot be resolved to a provider, when either cannot be priced, or when the baseline and the served model resolve to the same provider and model. That last case is model identity, not deployment identity, so routing between two deployments of one model reports zero even where their negotiated rates differ
...
One more correction runs on top: a baseline model with no cache-read or no cache-write rate of its own does not get those tokens for free, they become ordinary input at its plain rate. The relevant OpenAI, Azure, and Gemini entries have no separate cache-write rate, so those tokens are charged at the ordinary input rate. Treating an absent rate as zero would carry the whole prompt for nothing and turn a profitable route into a reported loss
...
`LiteLLM_SpendLogs` has no dedicated cache-token or savings columns, so the split cannot be queried as columns there. The request's logged usage, cache split included, rides in the row's `metadata` under `usage_object`, which is exactly what the daily savings writer reads back to rebuild `Usage`; the queryable, normalized split is the daily rollup above. Its `cache_hit` and `cache_key` columns are unrelated to any of this: they describe LiteLLM's own response cache, whether the proxy served this request from its cache rather than calling the provider, not provider-side prompt caching. What a single request does persist about routing is its `routing_decision` in metadata. `routed_model`, `cause` and `conversation_continuing` are always on it; `tier` appears when a tier was determined, and the baseline the deciding router derived appears alongside the deployment it resolved to
