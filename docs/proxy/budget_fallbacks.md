...
While the key is under its `anthropic-haiku-4-5` cap the request runs on `anthropic-haiku-4-5`. Once the cap is crossed subsequent requests are transparently served by `gpt-5.6-terra` without any `budget_exceeded` error surfacing to the caller.

### 3. Confirm the reroute

`/spend/logs?api_key=<sk-generated-key>` will attribute the post-fallback usage to `gpt-5.6-terra` (both the deployment and the `model_group`), so cost tracking, tagging, and per-model budgets remain accurate.
