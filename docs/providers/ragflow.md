...
Example: `ragflow/chat/my-chat-id/gpt-5.6-luna`
...
Example: `ragflow/agent/my-agent-id/gpt-5.6-luna`

Where:
- `{chat_id}` or `{agent_id}` is the ID of your chat or agent in RAGFlow
- `{model_name}` is the actual model name (e.g., `gpt-5.6-luna`, `gpt-5.6-terra`, etc.)
