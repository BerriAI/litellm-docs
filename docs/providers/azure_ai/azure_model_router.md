...
- **Cost Tracking**: LiteLLM automatically tracks costs based on the actual model used (e.g., `gpt-5.6-luna`), plus the Model Router infrastructure fee
...
Cost is tracked based on the actual model used (e.g., `gpt-5.6-luna`), plus a flat infrastructure cost of $0.14 per million input tokens for using the Model Router.
...
2. **Response model extraction**: Azure returns the actual model used in the response (e.g., `gpt-5.6-luna`). LiteLLM uses this for the model cost lookup.
