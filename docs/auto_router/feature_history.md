...
Merged after the v1.101.0 release candidate was cut. These ship in `v1.102.0-dev` builds; the stable link will be added when it ships.

- **Heuristic v1 tuning metered.** Existing heuristic settings are snapshotted on the first database-backed boot and keep serving. Without the `auto_router` license feature one router may change its tier mapping, weights, thresholds, or keyword lists; a second tuned router refuses to start from YAML and returns 403 from `/model/new`. [#39952](https://github.com/BerriAI/litellm/pull/39952).
- **Semantic auto-router cold start off the event loop.** The first-request route layer builds on a worker thread as one shared task, so a concurrent burst after a restart bills one embedding call and other traffic keeps flowing. [#39954](https://github.com/BerriAI/litellm/pull/39954).
- **Adaptive router fixes.** `model_info.input_cost_per_token` now counts toward cost-weighted scoring when `litellm_params` sets no price, in the standalone and hybrid paths. Persisted bandit state adds to the cold-start prior instead of replacing it, so one-sided feedback no longer leaves a request type returning 500 across restarts. [#39957](https://github.com/BerriAI/litellm/pull/39957), [#39955](https://github.com/BerriAI/litellm/pull/39955).
- **Cross-provider tool history on `/v1/messages`.** A tier switch from an OpenAI model to an Anthropic model no longer 400s on replayed `tool_use` blocks; LiteLLM-only fields stay off the native wire. [#39967](https://github.com/BerriAI/litellm/pull/39967).
...
- **Heuristic v2 classifier.** `classifier_type: heuristic_v2`, a calibrated local classifier with no LLM call on the request path. 27% more Terminal-Bench tasks solved at 45% lower cost per task than v1. [#39276](https://github.com/BerriAI/litellm/pull/39276). [Post](/blog/heuristic-v2).
- **Hybrid classifier.** `classifier_type: hybrid` keeps the heuristic scorer's tier and calls the LLM classifier only when the score lands within `hybrid_boundary_margin` of a tier boundary. [#39403](https://github.com/BerriAI/litellm/pull/39403).
- **Context-window escalation.** Oversized prompts move to the cheapest tier that fits before dispatch. On by default with `context_window_escalation_buffer: 0.95`; `enable_context_window_escalation: false` turns it off. [#38844](https://github.com/BerriAI/litellm/pull/38844), UI [#39054](https://github.com/BerriAI/litellm/pull/39054).
- **Modality routing.** Opt-in `modality_routing: true` sends image requests to a tier that can see them. `modality_pin_override` lets an image turn leave a pinned text-only model and return to it on the next text turn. [#39032](https://github.com/BerriAI/litellm/pull/39032), [#39454](https://github.com/BerriAI/litellm/pull/39454), UI [#39059](https://github.com/BerriAI/litellm/pull/39059).
- **User-turn classification.** `classification_mode: user_turn` classifies new user asks only and carries the decision through continuation turns; one "How often to classify" picker in the UI. [#38861](https://github.com/BerriAI/litellm/pull/38861), UI [#39042](https://github.com/BerriAI/litellm/pull/39042).
...
- **Classifier sees images.** Opt-in `classifier_llm_config.vision` (`enabled`, `max_images`) forwards request images to a classifier declared `supports_vision`, so a hard screenshot with a trivial caption routes on its content. [#39825](https://github.com/BerriAI/litellm/pull/39825), UI [#39840](https://github.com/BerriAI/litellm/pull/39840).
- **Classifier reasoning effort.** A classifier-only `reasoning_effort`, with allowed values drawn from the classifier model group; Test Connection probes with it before save. [#39372](https://github.com/BerriAI/litellm/pull/39372).
- **Classifier timeout and circuit breaker.** The classifier gets one total timeout with no retries or fallbacks, and a default-on circuit breaker sends other sessions straight to `classifier_fallback` for `circuit_breaker_cooldown_seconds` (default 30) after a timeout. [#39696](https://github.com/BerriAI/litellm/pull/39696), [#39701](https://github.com/BerriAI/litellm/pull/39701).
- **Tier health failover.** When every deployment of the decided tier model is cooled down, the request goes to a live peer in the same tier instead of returning 429, pinned sessions included; the decision logs `cause: health_failover`. [#39675](https://github.com/BerriAI/litellm/pull/39675).
- **Per-hop compression.** `auto_router_routing_compression` and `auto_router_model_compression` name a compression guardrail for the routing decision and for the model call separately, or `none` for either hop. Naming the same guardrail on both compresses once. [#39823](https://github.com/BerriAI/litellm/pull/39823). [Post](/blog/auto-router-per-hop-compression).
- **Shadow evals on teams, users, and model groups.** Target a `key`, `team`, or `user`, so JWT-authenticated traffic can be evaluated, and scope a job to a `models` list ANDed with those targets. [#39015](https://github.com/BerriAI/litellm/pull/39015), [#39828](https://github.com/BerriAI/litellm/pull/39828).
- **Shadow evals across several routers.** Compare up to four router configs via `router_names` on one job's sampled traffic, paired, with `results.by_router` win rates and spend. [#39028](https://github.com/BerriAI/litellm/pull/39028).
- **Shadow eval judges agentic traffic.** Tool-call turns are serialized and judged instead of dropped, and the judge output cap rises to 4096 tokens so a reasoning judge stops returning empty verdicts. [#39818](https://github.com/BerriAI/litellm/pull/39818), [#39817](https://github.com/BerriAI/litellm/pull/39817).
...
- **1M context preset; OpenAI Family preset on GPT-5.6.** The 1M Context preset runs Heuristic v2 with its complex tier on `gpt-5.6-sol`; the OpenAI Family preset moves to Luna, Terra, and Sol. [#39490](https://github.com/BerriAI/litellm/pull/39490), [#39396](https://github.com/BerriAI/litellm/pull/39396), [#39797](https://github.com/BerriAI/litellm/pull/39797).
- **Preset catalog served at runtime.** `GET /public/autorouter_presets` resolves the published catalog once per process (`LITELLM_AUTOROUTER_PRESETS_URL` to override, `LITELLM_LOCAL_AUTOROUTER_PRESETS=True` for the bundled copy), so preset updates need no dashboard rebuild. [#39412](https://github.com/BerriAI/litellm/pull/39412).
- **Sectioned prompt editor.** Editing a built-in classifier prompt saves only the opening instructions and calibration examples; the proxy derives tier criteria, labels, and guard, so a tier rename cannot disagree with the prompt. [#39688](https://github.com/BerriAI/litellm/pull/39688).
- **Session affinity TTL in the UI.** `session_affinity_ttl_seconds` (default 3600) from Advanced: Affinity. [#39679](https://github.com/BerriAI/litellm/pull/39679).
- **Claude Code fit.** Subagents follow the router their foreground session selected, read through Redis across workers, and `GET /v1/models` exposes the router's configured `model_info.mode`. [#39239](https://github.com/BerriAI/litellm/pull/39239), [#39619](https://github.com/BerriAI/litellm/pull/39619).
- **Routing embedding billed to the caller.** Each auto-routed request logs an `aembedding` row on the caller's key and team. [#39532](https://github.com/BerriAI/litellm/pull/39532).
- **Breaking.** The `auto_router` license feature meters customization: without it a proxy holds one `heuristic_v2` router and one router with `tier_definitions` or a custom classifier prompt; the shipped prompt, rubric presets, and `tier_labels` stay free ([#39468](https://github.com/BerriAI/litellm/pull/39468), [#39674](https://github.com/BerriAI/litellm/pull/39674)). Shadow eval renames `api_key_id` to `target_id`, `keys` to `targets`, and `key_alias` to `target_alias` in results ([#39015](https://github.com/BerriAI/litellm/pull/39015)).
