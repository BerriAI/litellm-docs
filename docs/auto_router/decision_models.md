---
title: Decision Model Classifiers
...
The gateway administrator configures each router's connections through YAML or the model-management API before users choose a provider in the editor. Existing Jev `api_base` and `api_key` settings retain their meaning. Laya uses separate `laya_api_base` and `laya_api_key` settings, so switching providers and saving keeps both connections available
...
Laya supports `multilingual`, `english`, and `typed-decisions`. The default is `multilingual`. Preload the selected checkpoint in the Laya server to avoid a cold model download on the first classification request. The example allows 60 seconds for a development CPU server; set the timeout to match the latency measured on your deployment
