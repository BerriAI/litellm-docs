...
The `openinference` mapper's `llm.input_messages.{idx}.*` and `llm.output_messages.{idx}.*` keys are the other unbounded family: two attributes per message, for the prompt and the response alike, on the same span. Past a few dozen turns they alone exceed the 128-attribute default and would evict the `gen_ai.*` model, usage, cost, and finish-reason attributes written before them. The per-index keys are therefore fitted to the attribute budget the span has left: the attribute count limit of the tracer provider that opened the span (`OTEL_SPAN_ATTRIBUTE_COUNT_LIMIT`, or the `SpanLimits` of a provider you inject, including a per-request routed one) minus every other attribute on the span, including the `error.*` attributes stamped on a failed call. A conversation that fits is indexed in full, whatever its length. When it does not fit, whole messages are dropped as role and content pairs, least valuable first: middle prompt turns, then extra response choices, then message 0 and the newest turn, and last the first choice, always under their original indices. How many turns survive depends on what else is on the span: with the `genai` and `openinference` mappers and `LITELLM_OTEL_LEGACY_COMPAT=false`, a 60-turn conversation with one reply at the default limit indexes `llm.input_messages.0`, `llm.input_messages.18` through `.59`, and `llm.output_messages.0`; with the `legacy` mapper on (the default) its extra keys eat into the same budget and fewer middle turns keep a key. The selection is by position, not role: message 0 and the newest prompt messages are the last prompt turns to lose a key of their own, which matters when `OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT` clips the `input.value` blob before it reaches the end of the conversation.

The cap only touches the per-index convenience keys. `input.value` and `output.value` still list every message's role and text, and the canonical `gen_ai.input.messages` and `gen_ai.output.messages` blobs carry the full message objects (tool calls and non-text parts included), so the whole conversation stays on the span and Arize keeps rendering it. Those blobs are single string attributes, so the SDK's `OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT` (unlimited by default) can cut their tail off on a long conversation; when it does, the per-index keys that survived the cap are what still shows the opener and the newest turns. Phoenix compacts the per-index keys into a dense list when it renders a span, so a gap in indices shows up there as a shorter message list, in the same order.
