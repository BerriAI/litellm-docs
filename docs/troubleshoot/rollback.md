...
- **`LITELLM_SALT_KEY`**: Do **not** change this value during rollback. It is used to encrypt/decrypt the credentials stored in the database (model and guardrail `litellm_params`, provider credentials, and the `environment_variables` and `router_settings` rows of the config table). Changing it will make existing credentials unreadable. See [Best Practices for Production](../proxy/prod#set-the-salt-key). Guardrail `litellm_params`, `router_settings` and the nested values of a model's `litellm_params` (an `extra_headers` dict, for example) saved by a version that encrypts them are ciphertext an older version does not decrypt, so after rolling back, save those rows again from the UI or the management API. See [Rolling upgrades and rollbacks](../proxy/security_encryption_faq#rolling-upgrades-and-rollbacks).
