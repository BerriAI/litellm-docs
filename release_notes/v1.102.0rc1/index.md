...
**Responses IDs are authorized regardless of their format.** An ID this proxy did not issue returns 403 on retrieve, cancel, delete, `input_items`, and `previous_response_id`, even for the response's owner. That covers raw provider IDs, IDs from before response ID encryption, and `response.queued` IDs saved from background streams before [PR #39534](https://github.com/BerriAI/litellm/pull/39534); proxy admin keys are exempt. The long IDs the proxy hands out keep working for their owner, so point clients at those. `general_settings.allow_unmanaged_response_ids: true` restores the old behavior for every key and is meant as an opt-out, not a fix. See [PR #39548](https://github.com/BerriAI/litellm/pull/39548)
