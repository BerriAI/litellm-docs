      ...
      },
      {
        ...
        encoding: {
          color: {
            ...
            scale: {
              domain: ['Python', 'Rust'],
              range: [colors.python, colors.rust],
            },
          },
          shape: {
            field: 'path',
...
function Chart({spec}: {spec: VisualizationSpec}) {
  return (
    <VegaEmbed className={styles.vega} spec={spec} options={embedOptions} />
  );
}
...
export function OcrThroughputChart(): ReactNode {
  return (
    <figure className={styles.chart}>
      ...
      />
      <figcaption>
        Six paired rounds on one CPU. Hover a point for its exact value. Source:
        the published primary-trial CSV.
      </figcaption>
    </figure>
  );
}
      ...
      <div className={styles.split}>
        <Chart
          spec={barSpec(
            ...
            '30 offered RPS · 8 MiB upload',
            [
              {
                path: 'Python',
                value: 17.6,
                note: 'Queued; varies across reruns',
              },
