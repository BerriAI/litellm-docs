  ...
  return (
    <svg
      ...
      <text
        ...
        fontSize="12"
        style={muted}>
        model: any-name-you-pick
      </text>

      <path
        d={`M ${REQ.x + REQ.w} ${clsMidY} L ${CLS.x - 2} ${clsMidY}`}
        style={{fill: 'none', stroke: PRIMARY, strokeWidth: 2.5}}
        markerEnd="url(#ar-primary)"
      />
      ...
      {TIERS.map((t, i) => {
        ...
        return (
          <g key={t.name}>
            <path
              d={curve(CLS.x + CLS.w, clsMidY, TIER.x - 2, mid)}
              style={arm}
              markerEnd={head}
            />
            <rect
            ...
            <text
              ...
              style={{fill: t.color, fontFamily: FONT}}>
              {t.name}
            </text>
            <text x={TIER.x + 32} y={y + 38} fontSize="12" style={muted}>
              {t.desc}
            </text>
            <path
              d={curve(TIER.x + TIER.w, mid, RES.x - 2, resMidY)}
              style={arm}
              markerEnd={head}
            />
          </g>
