...
The same check also keeps example model ids current. `docs-models.json` at the repo root is the source of truth: `current` maps a role (`openai_small`, `openai_large`, `anthropic`, `anthropic_large`, `gemini_pro`, `gemini_flash`) to the id examples should use, and `retired` lists older ids and the role each belongs to. `npm run lint:docs` fails with `retired-model` when a code block uses a retired id, and `python3 scripts/bump-docs-models.py docs` (also `npm run bump:models`) rewrites retired ids to the current one for their role inside code blocks and inline backticks, leaving tables, headings and prose alone. When the exact old id is the point of a block, such as model-specific behavior, a price map key, a regex or wildcard example, or a cache key, add `keep-model-ids` to the fence line (```yaml keep-model-ids) and both tools skip it. Prose has no fence line, so a sentence or list that must keep a retired id in backticks (a supported-model list, a note about one model) carries an MDX comment instead: `{/* keep-model-ids */}` at the end of a line, or `{/* keep-model-ids:start */}` and `{/* keep-model-ids:end */}` on their own lines around a list. The bump script skips those lines; the linter only checks fences. To move the docs to a new model, change the id in `current`, add the previous one to `retired`, and run the bump script.
