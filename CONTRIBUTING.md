...
To state which release a feature landed in, write `<SinceVersion v="1.85.0" />` where the note goes instead of a hand-written "Requires LiteLLM v1.85.0". It is registered globally as well, so no import is needed, and it renders a small "Since v1.85.0" pill that reads the same inline in a sentence, in a table cell, or on its own line under a heading. Give `v` the release name the way the changelog writes it (`1.85.0`, `1.79.4-stable`, `1.73.3-nightly`). `check-docs.py` fails a hand-written note under the version-note rule, so add `{/* keep-version-note */}` to the line when the version belongs to another project or the release itself is the subject rather than a feature's availability. The component lives in `src/components/SinceVersion/`
